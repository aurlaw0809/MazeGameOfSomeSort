...
class DynamicBackground():
    def __init__(self, screen, type):
        ...
        self.sky = pygame.image.load(f'assets/skies/{self.type}/1.png').convert_alpha()

        scale_factor = self.screen_height / self.sky.get_height()

        self.sky = pygame.transform.scale_by(self.sky, scale_factor)

        self.shoot = pygame.image.load(f'assets/skies/{self.type}/3.png').convert_alpha()
        self.shoot = pygame.transform.scale_by(self.shoot, scale_factor)

        self.back_cloud = pygame.image.load(f'assets/skies/{self.type}/2.png').convert_alpha()
        self.back_cloud = pygame.transform.scale_by(self.back_cloud, scale_factor)

        self.middle_cloud = pygame.image.load(f'assets/skies/{self.type}/4.png').convert_alpha()
        self.middle_cloud = pygame.transform.scale_by(self.middle_cloud, scale_factor)

        self.front_cloud = pygame.image.load(f'assets/skies/{self.type}/5.png').convert_alpha()
